...
def init_params_ols(model_list, ret_factor):
	...
	for model in model_list:
		...
		factor_returns.append(ret_factor[model.params.index].values)
...
def e_step_theta(initial_theta, alpha_list):
	...
	for i in range(MN):
		Demon = 0.0
		for j in range(L):
			Demon += pi[j]*np.sqrt(1.0/2*sigma[j])*np.exp(-((alpha_list[i]-u[j])**2)/(2*sigma[j]))
		for j in range(L):
			number = pi[j]*np.sqrt(2*sigma[j])*np.exp(-((alpha_list[i]-u[j])**2)/(2*sigma[j]))
			if Demon != 0:
				P[i,j] = number / Demon
			else:
				P[i,j] = 0.0
...
def m_step_theta(P, alpha_list):
	theta = np.zeros([3,P.shape[1]])
	for j in range(P.shape[1]):
		mu_k = 0.0
		Demon = 0.0
		mu_D= 0.0
		for i in range(P.shape[0]):
			mu_k += P[i,j] * alpha_list[i]
			Demon += P[i,j]
		theta[1, j] = mu_k / Demon

		for i in range(P.shape[0]):
			mu_D += P[i,j] * ((alpha_list[i]-theta[1, j])**2)
		theta[2 ,j] = mu_D / Demon
		theta[0, j] = Demon / P.shape[0]
	return theta


def em_algorithm(initial_theta, alpha_list, epsilon = 0.0001, iter_num = 500):
	theta = initial_theta
	for i in range(iter_num):
		old_theta = np.copy(theta)
		P = e_step_theta(theta, alpha_list)
		theta = m_step_theta(P, alpha_list)
		print old_theta
		print theta
		print i
		print np.sum(abs(theta - old_theta))
		print "=============================================================="
		if np.sum(abs(theta - old_theta)) < epsilon:
			print "第%d次迭代结束"%i
			break
	return theta
"""
